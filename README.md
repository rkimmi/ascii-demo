...
## Batch via CLI (to support video)
...
```bash
ffmpeg -framerate 24 -i frame_%04d.png -vf "eq=saturation=1.4,scale=in_range=full:out_range=full:out_color_matrix=bt709,format=yuv444p" -c:v libx264 -crf 14 -preset slow -colorspace bt709 -color_primaries bt709 -color_trc bt709 -color_range pc -movflags +faststart output-fullrange-444.mp4
```
